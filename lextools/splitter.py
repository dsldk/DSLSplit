...
class Splitter2:
    ...
    def __init__(self, ngram_probs: dict = de_ngram_probs, language: str = "de", lemma_list: List | None = None):
    ...
    def split_compound(self, word: str) -> List[Tuple[float, str, str]]:
        ...
        def cut_fuge(word_slice: str, language: str) -> tuple:
            ...
            if language == "de":
                ...
                end_slices = ["teks", "ions", "ngs", "ms", "ns", "ds", "bs", "vs", "ls", "rs", "js"]
            ...
            for k in range(len(word) + 1, 2, -1):
                ...
                if not pre_slice_prob and k <= len(pre_slice):
                    ...
                    pre_slice_prob.append(
                        self.ngram_probs["suffix"].get(end_ngram, -1))  # Punish unlikely pre_slice end_ngram

                # Probability of ngram in word, if high, split unlikely
                in_ngram = word[n:n + k]
                in_slice_prob.append(
                    self.ngram_probs["infix"].get(in_ngram, 1))  # Favor ngrams not occurring within words
